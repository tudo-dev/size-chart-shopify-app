...
export function buildReaderPrompt(input: Pick<ReadInput, 'productTitle' | 'productType' | 'store'>): { system: string, user: string } {
  const system = [
    ...
    '9. "notes": short supplier notes a customer should see, translated to plain English (measurement tolerance such as "measured flat by hand, allow 1-3 cm", fit advice such as "slim fit, size up for a loose fit"). Skip marketing text and care symbols.',
