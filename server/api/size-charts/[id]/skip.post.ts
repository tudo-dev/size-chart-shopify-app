/**
 * Skip a chart: a person decided this picture is not to be published.
 *
 *   POST /api/size-charts/:id/skip    body: { note?: string }
 */

import { getVerifiedShopUrl } from '../../../utils/shopify'
import { skipChart, chartRowById } from '../../../utils/size-chart-store'
...
export default defineEventHandler(async (event) => {
  try {
    ...
    const result = skipChart(id, shop, note)
    ...
    console.error('Error on a size chart (skip):', error)
