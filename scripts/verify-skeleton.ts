...
// The settings only, without the # comments that explain them.
const site = read('deploy/nginx/sizecharts.tudoholic.com.conf').split('\n').map(line => line.replace(/#.*$/, '')).join('\n')
const deployScript = read('deploy/deploy_sizecharts')
check('the nginx site sends traffic to the port the container listens on, and the deploy script checks that same port',
  [/proxy_pass http:\/\/127\.0\.0\.1:(\d+);/.exec(site)?.[1], /127\.0\.0\.1:(\d+)\/api\/health/.exec(deployScript)?.[1]].map(Number), [hostPorts[0], hostPorts[0]])
check('  it has no shared cache: every /api answer belongs to the login that asked (the Logistics site caches /api for 10 s)', /proxy_cache/.test(site), false)
check('  it lets the sheet through (nginx refuses over 1 MB by default; the app allows 20 MB)', Number(/client_max_body_size (\d+)m;/.exec(site)?.[1] ?? 0) >= 20, true)
check('  it passes the caller\'s address and the host on, like the Logistics site', ['proxy_set_header Host $host;', 'proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;', 'proxy_set_header X-Forwarded-Proto $scheme;'].every(line => site.includes(line)), true)
check('  and the deploy script stops before touching anything when .env is missing', deployScript.indexOf('if [ ! -f .env ]') < deployScript.indexOf('git pull'), true)
